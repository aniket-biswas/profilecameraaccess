...
export default function Profile() {
    const [profilePicUri, setProfilePicUri] = useState(require("../../assets/favicon.png"));
    ...
    async function fetchUsersData() {
        ...
        if (loggedUserId) {
            ...
            usersDbRef
                ...
                .once('value')
                .then(resp => {
                    const response_data = resp.val();
                    console.log(response_data);
                    if (response_data) {
                        //for getting performance pie chart
                        var total=0,correct=0,incorrect=0;
                        const quizResponses=response_data.quizResponses || {};
                        ...
                        for(const quizId in quizResponses) {
                            ...
                            const response = quizResponse.responses || {};
                            const x=Object.keys(response).length || 0;
                            total+=x;
                            for (const questionId in response) {
                                const ansResponse = response[questionId];
                                if(ansResponse.isCorrect){
                                    correct++;
                                }
                            }
                        ...
                        setDesc(response_data.desc);
                        setAgeGroup(response_data.ageGroup);
                        ...
                        if(response_data.profilePicUri){
                            setProfilePicUri(response_data.profilePicUri);
                        }
    ...
    function updateProfileInFirebase(loggedUserId, imageUploadUrl) {
       ...
       (error)=>
       {
           if(error){
               setIsLoading(false);
               displaySnackBar("error","failed to upddate profile")
           }
           else{
               setIsLoading(false);
               displaySnackBar("success","profile updated successfully")
           }
       })
        //write code
    }
